...
* YouTubeで戦闘＆ML-Agentsジムを見る - <https://youtu.be/placeholder-gym] (パスワード：level5)
* YouTubeでゲームプレイを見る - <https://youtu.be/placeholder-gameplay>
...
### 🚀 技術的課題
* **複雑なRPG設計とデータ駆動型拠点シミュレーション:** ScriptableObjectを活用した39名のユニーク英雄と18種類の上位クラス設計。施設への人員配置（農場、酒場、工房、訓練所）、装備の耐久度・鍛造、部隊シナジー、序列システムを備えた自律的な拠点シミュレーション。
* **リアルタイム戦術戦闘とML-Agents訓練環境:** タンクのヘイト上限管理（後衛への回り込みを可能にする最大2体制限）、残HP比率を優先するヒーラーAI、ボスの攻撃予兆表示（テレグラフ）を実装。8次元の正規化観測ベクトルを備え、10倍速で動作する強化学習用の独立アリーナ（`Gym_Combat`）を構築。
